...
primary_artifact: .ai-operation/docs/project_map/activeContext.md
---
> ⚠️ **议题 #013 物理强制(必读)**:本 skill 修改 activeContext.md(议题 #002 The Engine 物质载体)。议题 #013 强制:必须用 aio__skill_invoke / aio__skill_complete 包裹执行。
>
> 执行流程:`aio__skill_invoke("state-checkpoint")` → 走完本 skill 所有步骤 → `aio__skill_complete("state-checkpoint")`。

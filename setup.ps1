...
"@ 2>&1
    ...
    foreach ($f in @("systemPatterns","techContext","activeContext","corrections","inventory")) {
    ...
    if ($pmFiles -eq 5) { Check-Pass "All 5 project_map files present" }
    else { Check-Fail "Only $pmFiles/5 project_map files found" }
    if ($pmFilled -ge 3) { Check-Pass "$pmFilled/5 files have content" }
    else { Check-Warn "Only $pmFilled/5 files filled - run [初始化项目]" }

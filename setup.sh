...
" 2>&1 || echo "FAIL")
    ...
    PM_FILES=0
    PM_FILLED=0
    for f in systemPatterns techContext activeContext corrections inventory; do
        pf="$PM_DIR/${f}.md"
        if [ -f "$pf" ]; then
            PM_FILES=$((PM_FILES + 1))
    ...
    if [ "$PM_FILES" -eq 5 ]; then
        check_pass "All 5 project_map files present"
    else
        check_fail "Only $PM_FILES/5 project_map files found"
    fi
    if [ "$PM_FILLED" -ge 3 ]; then
        check_pass "$PM_FILLED/5 files have content (not template)"
    else
